...
const navigationSections = [
  {
    ...
    links: [
      { label: 'Corporate & Commercial Litigation', href: '#services' },
      { label: 'Energy & Infrastructure Projects', href: '#services' },
  ...
  },
  {
    title: 'Firm',
    links: [
      { label: 'About Lex Alliance', href: '#about' },
  ...
  },
  {
    title: 'Insights',
    links: [
      { label: 'Client Alerts', href: '#insights' },
...
const contactChannels = [
  {
    icon: PhoneIcon,
    label: 'Call us',
    value: '+233 557 618 757',
    href: 'tel:+233557618757',
  },
  {
    icon: MailIcon,
    label: 'Email',
    value: 'legal@lexalliance.com',
    href: 'mailto:legal@lexalliance.com',
  },
  {
    icon: MapPinIcon,
    label: 'Visit',
    value: '17 Capitol Street, Accra',
...
const Footer = () => {
  return (
    <footer className='relative overflow-hidden bg-slate-900 text-slate-300'>
      <div className='pointer-events-none absolute inset-0 bg-linear-to-tr from-slate-900 via-slate-800 to-slate-900 opacity-95' />
      <div className='pointer-events-none absolute left-1/2 top-10 h-56 w-56 -translate-x-1/2 rounded-full bg-amber-500/10 blur-3xl' />
      ...
      <div className='relative mx-auto max-w-7xl px-6 py-16 sm:px-8 lg:px-10'>
        <div className='flex flex-col gap-12 lg:flex-row lg:gap-16'>
          <div className='flex-1'>
            ...
            <p className='mt-5 max-w-lg text-sm leading-relaxed text-slate-400'>
              We partner with ambitious leaders, institutions, and investors to navigate
              complex legal landscapes across Africa. Our counsel blends deep sector
              expertise with strategic foresight to resolve disputes and unlock growth.
            </p>
            ...
            <div className='mt-6 flex flex-wrap items-center gap-3'>
              {socialLinks.map(({ icon: Icon, href }) => (
                <a
                  ...
                  target='_blank'
                  rel='noreferrer'
                  className='flex h-10 w-10 items-center justify-center rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-400 transition-all duration-300 hover:-translate-y-0.5 hover:border-amber-400 hover:bg-amber-500/30 hover:text-amber-300'
                >
                  <Icon />
                </a>
              ))}
          ...
          <div className='grid flex-1 gap-10 sm:grid-cols-2 lg:flex lg:flex-[1.5] lg:justify-between'>
              ...
              <div className='mt-4 space-y-4 text-sm text-slate-400'>
                {contactChannels.map(({ icon: Icon, label, value, href }) => (
                  <a
                    key={label}
                    href={href}
                    className='group flex items-start gap-3 transition-colors duration-200 hover:text-amber-400'
                  >
                    <span className='mt-1 inline-flex h-8 w-8 items-center justify-center rounded-full border border-amber-500/30 bg-amber-500/10 text-amber-400'>
                      <Icon />
                    </span>
                    <span>
                      <span className='block text-xs uppercase tracking-[0.2em]'>{label}</span>
                      <span className='mt-1 block text-base text-slate-200'>{value}</span>
                    </span>
                  </a>
                ))}
              </div>
        ...
        <div className='mt-12 flex flex-col gap-4 border-t border-white/5 pt-8 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between'>
          <p>© {new Date().getFullYear()} Lex Alliance Legal Partners. All rights reserved.</p>
          <div className='flex flex-wrap items-center gap-5'>
            <Link href='#privacy' className='transition-colors duration-200 hover:text-amber-400'>
              Privacy Notice
            </Link>
            <span className='hidden h-3 w-px bg-slate-700 sm:inline-block' />
            <Link href='#terms' className='transition-colors duration-200 hover:text-amber-400'>
              Terms of Engagement
            </Link>
            <span className='hidden h-3 w-px bg-slate-700 sm:inline-block' />
            <Link href='#disclaimer' className='transition-colors duration-200 hover:text-amber-400'>
